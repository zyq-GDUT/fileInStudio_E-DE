...
********************************************************************/
//i指标函数
double I_index(double k_num, double *p_pars, double *p_index, int *in_clu, int N, int D){
...
int e_de(double *p, double *p2, int N, int D, int Gmax, double *Xl, double *Xu) {
	...
	for (k = 0; k<Gmax; k++)      //Gmax denotes the maximum iteration times
	{
		for (i = 0; i<NP; i++)   
		{
			...
			//还需要把具体样本对应于哪个簇复制过来。in_cluster->incluster2
...
int  main()
{
	...
	data = loadData(&D, &N);   //load data from the text file,将数据集存到了二维数组data中
