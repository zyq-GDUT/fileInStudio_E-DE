...
* 该方法输出单次测试的数据
*/
void outputExperimentdata1(trailData& traildata) {
	...
	for (int i = 0; i < MAX_TEST_NUM; i++) {
		if (i == 0) {
			printf("bestClusterNum:  ");
		}
		printf("%12d ", traildata.bestClusterNum[i]);
	...
	for (int i = 0; i < MAX_TEST_NUM; i++) {
		...
		printf("%10.4f ", traildata.bestfitness[i]);

	}
	printf("\n");
	for (int i = 0; i < MAX_TEST_NUM; i++) {
		if (i == 0) {
			printf("IntraDistance：\t");
		}
		printf("%10.6g ", traildata.Intradist_arr[i]);

	}
	printf("\n");
	for (int i = 0; i < MAX_TEST_NUM; i++) {
		if (i == 0) {
			printf("InterDistance：\t");
		}
		printf("%10.6g ", traildata.Interdist_arr[i]);
	...
	for (int i = 0; i < MAX_TEST_NUM; i++) {
		...
		printf("%10.6f ", traildata.ARI_arr[i]);
...
*/
void outputExperimentdata2(double avgARI, double avgInterDist, double avgIntraDist, double avgfitness, int* clusterCount, double stdARI, double stdInterDist, double stdIntraDist, double stdfitness) {
	printf("\n%d次测试的聚类情况[簇数(出现次数)]：", MAX_TEST_NUM);
	for (int i = 0; i < MAX_CLUSTER+1; i++) {
		if (clusterCount[i] == 0) continue;
		printf("\t%d(%d) ", i, clusterCount[i]);
	}
	printf("\n");
	printf("平均适应度(AVG.Fitness): %g\t标准差: ±%g\n",avgfitness,stdfitness);
	printf("平均簇内距离(Intra Distance): %g\t标准差: ±%g\n", avgIntraDist,stdIntraDist);
	printf("平均簇间距离(Inter Distance): %g\t标准差: ±%g\n", avgInterDist,stdInterDist);
	printf("平均ARI值(AVG.ARI): \t%10g\t\t标准差: ±%g\n", avgARI,stdARI);
...
void manageExperimentData(trailData& traildata) {
	...
	outputExperimentdata1(traildata);
	outputExperimentdata2(avgARI, avgInterDist, avgIntraDist, avgfitness, clusterCount, stdARI, stdInterDist, stdIntraDist, stdfitness);
