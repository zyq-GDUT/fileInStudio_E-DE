...
maxClusterNum是最大的簇数
*/
void outputPopulationInfo(double* p,double* p2,int D,int np,int maxClusterNum,int** incluster,int N, FILE* outputfile) {
	...
	for (int i = 0; i < np; i++) {
		...
		for (int j = 0; j < k_num; j++) {
			fprintf(outputfile, "  %d号簇,%d个样本：",j,c[j]);
			fprintf(outputfile, "[");
			for (int l = 0; l < D; l++) {
				fprintf(outputfile, "%g", *(p + i * maxClusterNum * D + j * D + l));
				if (l != D - 1) {
					fprintf(outputfile, ",");
				}
				else{
...
* gap是指每隔gap代输出一次
*/
void outputPopulationInfo2(double* p, double* p2, int D, int np, int maxClusterNum, int** incluster, int N,int generation,double bestfitness,double bestclusternum,int gap,FILE* outputfile) {

	fprintf(outputfile,"\n---------第%d代-最优适应度为%g-对应的簇数为%d------------------\n", generation + 1, bestfitness, (int)bestclusternum);
	if (generation % gap == 0) {
		outputPopulationInfo(p, p2, D, np, maxClusterNum, incluster, N, outputfile);
