...
int e_de(double* p, double* p2, int N, int D, int Gmax, double* Xl, double* Xu,int usedIndex,int **in_cluster) {
	...
	for (k = 0; k < Gmax; k++)      //Gmax denotes the maximum iteration times
	{
		...
		for (i = 0; i < NP; i++)
		{
			...
			//if (mutate_u > max_cluster_num) //变异向量中一个个体的簇中心数的取值范围是2~20
			...
			for (int j = 0; j < max_cluster_num; j++) { //这三个数组用于下面的变异处理
				r1_rand[j] = 0;
				r2_rand[j] = 0;
				r3_rand[j] = 0;
			}
			...
			MU = 0.10 - 0.06 * k / ((double)Gmax - 1.0); //上面的操作是得到初始的变异个体，该步骤是对初始变异个体按一定概率进行处理，每轮迭代添加高斯扰动处理的变异个体数大概是NP*MU
			...
			//需要杂交的簇数大于1，即需要使用子空间杂交法，首先需要得到子空间的均值向量（中心向量）
				...
				//		}
				//	}
				//	if (swap_flag == 1) {//把不需要被覆盖的簇保存起来，即这部分簇是目标个体中不需要被变异个体替换的簇。随后这部分簇与变异个体中被选中的杂交簇结合成试验向量
				//		for (int l = 0; l < D; l++)
